...
cd "$archivedir" || {echo "Unable to cd into $archivedir"; exit}

show_deleted_files(){
   find . ! -path . | sed -e 's/\([[:digit:]][[:digit:]]\.\)\{5\}//g' \
    -e 's/^/  /'
}

choose_version_restore(){
  index=1
  for name in $(ls -ts *"$1")
  do
...
# If no args, just show listing of deleted files
if [ $# -eq 0 ] ; then
  $(show_deleted_files)
  exit 0
fi
...
if [ $matches -gt 1 ] ; then
  ...
  index=$(choose_version_restore)

  echo ""

  desired=$(prompt_user_choose_version)
